import { getOverview, getDaily, getRecentEvents } from "@/lib/stats";
import {
  Card,
  StatCard,
  PageHeader,
  ...
  return (
    <div>
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-xs text-slate-400">
              {overview.lastActivityAt
                ? `last activity ${fmtDate(overview.lastActivityAt)}`
                : "no activity yet"}
            </span>
          </div>
        }
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-4">
        <StatCard label="Leads found" value={overview.leadsFound} accent="violet" />
        <StatCard
          label="Emails generated"
          value={overview.emailsGenerated}
          accent="slate"
        />
        <StatCard label="Emails sent" value={overview.emailsSent} accent="blue" />
        <StatCard
          label="Follow-ups sent"
          value={overview.followupsSent}
          accent="violet"
        />
        <StatCard
          label="Replies"
          value={overview.repliesDetected}
          accent="green"
        />
        <StatCard
          label="Bounces"
          value={overview.bouncesDetected}
          accent="red"
        />
        <StatCard label="Failures" value={overview.failures} accent="amber" />
        <StatCard label="Runs / syncs" value={overview.runs} accent="slate" />
      </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <Card>
          ...
          <RateRow
            label="Reply rate"
            num={overview.repliesDetected}
            den={overview.emailsSent}
            color="bg-emerald-500"
          />
          <RateRow
            label="Bounce rate"
            num={overview.bouncesDetected}
            den={overview.emailsSent}
            color="bg-red-500"
          />
          <RateRow
            label="Follow-up rate"
            num={overview.followupsSent}
            den={overview.emailsSent}
            color="bg-violet-500"
          />
          <RateRow
            label="Failure rate"
            num={overview.failures}
            den={overview.emailsSent + overview.failures}
            color="bg-amber-500"
          />
        </Card>
      </div>
